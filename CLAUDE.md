...
- **Records** (`loadRecords`/`saveRecords`/`qualifies`/`insertEntry`/`renderRecords`): top 5 persisted in `localStorage` under `tetris-records` as `{ entries: [{name, score, lines, level, combo, date}], bestCombo, maxLines }`. `loadRecords` sanitizes and re-sorts whatever it reads, so a corrupt value degrades to an empty table instead of throwing. Rendered into both `#start-records` (start screen) and `#overlay-records` (game over), with the qualifying row highlighted.
- **Combo**: `lockPiece` bumps `combo` on any clearing lock and zeroes it otherwise; `maxCombo` is the per-game best and feeds `bestCombo` in storage. Combo does not affect scoring.
- **Screens**: `resetGame()` rebuilds state and draws one frame without running; `init()` (Jugar/Reiniciar) starts the loop; `showStartScreen()` (boot/Menú) resets and shows `#start-overlay`. The `started` flag gates keyboard input and pause.
- **Skins** (`SKIN_COLORS`/`SKIN_DRAW`/`applySkin`): four palettes + block styles (`retro`, `neon`, `pastel`, `pixel`) picked in `#skin-select` and persisted under `tetris-skin`. `applySkin` reassigns `COLORS` (declared `let`, not `const`) and sets `body.skin-<name>` so the CSS can override the theme variables; every overlay/menu inherits the skin because they all style from those variables.

Tunable constants sit at the top of `game.js`: `COLS`, `ROWS`, `BLOCK`, `SKIN_COLORS`, `LINE_SCORES`, `MAX_START_LEVEL`, `MAX_RECORDS`. If `COLS`/`ROWS`/`BLOCK` change, update the `#board` canvas `width`/`height` in `index.html` to match (`COLS × BLOCK`, `ROWS × BLOCK`).
